    ...
    <list-layout>
      ...
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        ...
        >
          ...
          <el-table-column
            prop="manufactoryCode"
            label="生产厂商编码"
          ></el-table-column>
          <el-table-column
            prop="manufactoryName"
            label="名称"
          ></el-table-column>
...
export default {
  ...
  methods: {
    getAuthList() {
      this.request.manufactoryGetList().then(result => {
        this.tableData = result;
      });
    },
    search(data) {
      if (data.id) {
        this.request.manufactorySearch(data.id).then(result => {
          this.tableData = [];
    ...
    addItem(form) {
      ...
      this.request.manufactoryInsert(form).then(() => {
        this.dialogVisible = false;
        this.getAuthList();
      });
    },
    ...
    dele() {
      ...
      this.request
        .manufactoryDeleteById(this.multipleItemSelection[0].id)
        .then(() => {
          this.getAuthList();
        });
    },
    ...
    changeItem(form) {
      ...
      this.request.manufactoryUpdate(form).then(() => {
        this.dialogVisible = false;
        this.getAuthList();
      });
    }
