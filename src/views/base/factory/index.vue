      ...
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        ...
        >
          ...
          <el-table-column
            ...
            label="厂商编码"
          ></el-table-column>
          <el-table-column prop="factoryName" label="名称"></el-table-column>
...
export default {
  ...
  methods: {
    getAuthList() {
      this.request.factoryGetList().then(result => {
        this.tableData = result;
      });
    },
    search(data) {
      if (data.id) {
        this.request.factorySearch(data.id).then(result => {
          this.tableData = [];
    ...
    addItem(form) {
      ...
      this.request.factoryInsert(form).then(() => {
        this.dialogVisible = false;
        this.getAuthList();
      });
    },
    ...
    dele() {
      ...
      this.request
        .factoryDeleteById(this.multipleItemSelection[0].id)
        .then(() => {
          this.getAuthList();
        });
    },
    ...
    changeItem(form) {
      this.request.factoryUpdate(form).then(() => {
        this.dialogVisible = false;
        this.getAuthList();
      });
    }
